  ...
  <div class='jyfp'>
    ...
    <div class="fbox_btn fbox_btn2">  
      ...
      <el-select v-model="Difficulty" placeholder="选择难度">
        <el-option
          class="hard_select"
          v-for="item in Difficulty_level"
          :key="item.value"
          :label="item.label"
          :value="item.value">
        </el-option>
      ...
      <el-button @click="changehard(Difficulty)">新游戏</el-button>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    changehard(){
      if(this.Difficulty==undefined){
        this.Difficulty = this.fbox.hard ? this.fbox.hard : 1;
      }

      if(this.Difficulty){
        //修改难度
        this.fbox.set({
          hard: this.Difficulty,
        });
...
//生命周期 - 挂载完成（可以访问DOM元素）
  mounted() {
    this.fbox = new Findpair('fbox','output');
