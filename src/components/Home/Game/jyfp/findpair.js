...
export function Findpair(divid,output,config){
  if(!divid){
    return false;
  }
  ...
  fbox.newgame = function(){
    ...
    for(let i=0;i<pairs;i++){
      const rander = randomNum(1,20,icons);
      icons.push(rander);
    }

    //分配随机数组
    const total = row*row/2;
    const averarr = average(total,pairs);
    ...
    for(let i=0;i<rowarr.length;i++) {
      ...
      td1.style.backgroundImage = td2.style.backgroundImage = 'url("static/images/gameImage/jyfp/'+sarr[2]+'.png")';
    }

    //添加点击事件
    var tout;
    var li = fboxid.getElementsByTagName('li');
    ...
    for(let i=0;i<li.length;i++){
      li[i].addEventListener('click', function() {
        ...
        if(data1==null && data2==null){
          ...
          data2 = lid;
          var bg1 = fboxid.querySelector('[data-fid="'+data1+'"]').getElementsByTagName('p')[0].style.backgroundImage;
          var bg2 = fboxid.querySelector('[data-fid="'+data2+'"]').getElementsByTagName('p')[0].style.backgroundImage;
          if(bg1==bg2){ //成功配对
          ...
          if(outputdiv){
            ...
            var prohtml = _this.success+'/'+total;
        ...
        if(_this.success==total){
          ...
          var overtime = _this.times2 - _this.times1;
          var ifbest = '，总用时<span style="color:red">'+formatTime(overtime)+'</span>';
